...
library(DescTools)  # Para R² de Nagelkerke
library(lmtest)     # Para teste de razão de verossimilhança
library(plotly)
library(knitr)
library(markdown)

#Extraindo da API da Câmara
{
  dados_agregados_parlamentares <- data.frame()  # data frame vazio
  
  #Coleta dos parlamentares entre 2001 e 2022
  for (ano in 2001:2022) {
    url <- paste0("http://dadosabertos.camara.leg.br/arquivos/votacoesVotos/json/votacoesVotos-",ano,".json")
    json_data <- fromJSON(url)
    votos <- json_data$dados
    votos$ano <- ano
    dados_agregados_parlamentares <- bind_rows(dados_agregados_parlamentares, votos)
  }
  
  #Seleção dos IDs únicos de parlamentares e agregação num data frame
  dados_agregados_parlamentares <- unique(dados_agregados_parlamentares$deputado_)
  ids_parl <- data.frame(dados_agregados_parlamentares$id)
  
  migrações <- data.frame() # data frame vazio
  
  #Coleta do histórico de cada parlamentar i do data frame de 2001 a 2022
  for (i in seq_len(nrow(ids_parl))){
    id_parlamentar <- ids_parl$dados_agregados_parlamentares.id[i]
    url_migr <- paste0("https://dadosabertos.camara.leg.br/api/v2/deputados/", id_parlamentar, "/historico")
    dados_em_json <- fromJSON(url_migr)
    migrações <- bind_rows(migrações, dados_em_json$dados)
    #Sys.sleep(0.2) # pausa na requisição para evitar sobrecarregamento da API
  }
  
  #Seleção das colunas interessantes para análise
  migrações_filtrado <- migrações |> 
    select(dataHora, id, nome, nomeEleitoral, 
           siglaPartido,siglaUf, idLegislatura, 
           situacao, condicaoEleitoral, descricaoStatus)
  
  migrações_filtrado$nome <- str_to_title(migrações_filtrado$nome)
  migrações_filtrado$nomeEleitoral <- str_to_title(migrações_filtrado$nomeEleitoral)
  
  situações_migrantes <- data.frame(table(migrações_filtrado$descricaoStatus))
...
#Puxando e transformando os dados
{
  ...
  ver_releitos <- banco_final_cap |> 
    filter(idLegislatura >= 55)
}

#Line plots
{
  número_abs_migrações <- ggplot(
    data = banco_de_migrações |>
      count(Ano_Eleição, Ideologia),  # Faz o agrupamento e a contagem direto aqui
    
    aes(x = Ano_Eleição, y = n, color = Ideologia, group = Ideologia)
  ) +
    geom_line(size = 1) +
    geom_point() +
    labs(
      title = "Quantidade de Parlamentares por Ideologia e Eleição",
      x = "Ano da Eleição",
      y = "Parlamentares Migrantes",
      color = "Ideologia"
    ) +
    theme_minimal()
  ...
  ggplot(
    banco_de_migrações |>
      group_by(Ano_Eleição, Ideologia) |>
      summarise(n = n(), .groups = "drop") |>
      group_by(Ano_Eleição) |>
      mutate(porcentagem = 100 * n / sum(n)),
    aes(x = Ano_Eleição, y = porcentagem, color = Ideologia, group = Ideologia)
  ) +
    geom_line(size = 1.5) +
    geom_point(shape = 21, color = "black", size = 3.5, fill = "#000000", alpha = 0.7) +
    labs(
      title = "Porcentagem de Parlamentares por Ideologia e Eleição",
      x = "Ano da Eleição",
  ...
  número_abs_migrações+percentual_migrações
  número_abs_migrações/percentual_migrações
  ...
  var_bancada <- ggplot(
    ...
    geom_text(aes(label = paste0(round(porcentagem, 1), "%")), 
              vjust = -1.7, size = 3.5, family = "Garamond", color = "black") +  # rótulos
    scale_color_manual(values = c(
      "Esquerda" = "#e2030a",
      "Centro"   = "#0a6b00",
      "Direita"  = "#0073e6"
    )) +
    scale_fill_manual(values = c(  # manter o preenchimento igual à borda
      "Esquerda" = "#e2030a",
      "Centro"   = "#0a6b00",
      "Direita"  = "#0073e6"
    )) +
    geom_vline(xintercept = c(2002, 2005, 2007, 2015), linetype = "dashed", color = "orange", size = 1) +
    annotate("text", x = 2002, y = 12, label = "Lula se elege
      (2002)",
             angle = 0, vjust = -0.5, hjust = 0, size = 3.5, family = "Garamond") +
    annotate("text", x = 2005, y = 65, label = "Mensalão
   (2005)", 
             angle = 0, vjust = -0.5, hjust = 0, size = 3.5, family = "Garamond") +
    annotate("text", x = 2007, y = 35, label = "STF decide que mandato 
  proporcional é do partido
                (2007)",
             angle = 0, vjust = -0.5, hjust = 0, size = 3.5, family = "Garamond") +
    annotate("text", x = 2015, y = 27, label = "STF decide que mandato 
  majoritário é do político
              (2015)",
             angle = 0, vjust = -0.5, hjust = 0, size = 3.5, family = "Garamond") +
    labs(
      title = "Percentual de Parlamentares por Ideologia e Eleição (reeleitos e não reeleitos)",
      x = "Ano da Eleição",
      y = "Variação da bancada (nº abs)",
  ...
  banco_de_migrações <- banco_de_migrações |>
    group_by(Nome_Deputado) |>
    mutate(Reeleitos = ifelse(n() > 1, "Sim", "Não")) |>
    ungroup()
  
  reeleitos <- ggplot(
    banco_de_migrações |>
      filter(Reeleitos == "Sim") |> 
      group_by(Ano_Eleição, Ideologia) |>
      summarise(n = n(), .groups = "drop") |>
      group_by(Ano_Eleição) |>
      mutate(porcentagem = 100 * n / sum(n)),
  ...
  majoritário é do político
             ...
             angle = 0, vjust = -0.5, hjust = 0, size = 3.5, family = "Garamond") +
    labs(
      title = "Percentual de Parlamentares por Ideologia e Eleição (apenas reeleitos)",
      x = "Ano da Eleição",
      y = "Variação da bancada (%)",
      color = expression(bold("Ideologia"))) +
    theme_classic(base_family = "Garamond") +
    theme(
      plot.title = element_text(face = "bold", size = 16, margin = margin(b = 20)),
  ...
  line_plot_migração <- var_bancada+reeleitos
  ggsave("var_bancada_line_plot.png", plot = line_plot_migração, width = 22, height = 8, limitsize = FALSE, units = "in", dpi = 1000)
}
...
#Sankey plots
{
  ...
  # 2. Função principal para gerar Sankey por Ideologia com cores fixas
  {
    gerar_sankey_ideologia_colorido <- function(df) {
      ...
      # Extrair ideologia de cada node
      nodes$ideologia <- gsub("_.*", "", nodes$name)
      
      # Atribuir cor com base na ideologia
      nodes$color <- recode(nodes$ideologia,
                            Esquerda = "#e2030a",
                            Centro   = "#0a6b00",
                            Direita  = "#0073e6")
      
      # Mapear índices para source/target
      df_pairs$source_id <- match(df_pairs$source, nodes$name) - 1
      df_pairs$target_id <- match(df_pairs$target, nodes$name) - 1
      
      df_pairs$group <- nodes$ideologia[df_pairs$source_id + 1]
      
      # Criar sankey com estilo
      sankey <- sankeyNetwork(
        ...
        NodeGroup = "ideologia",
        LinkGroup = "group",
        fontSize = 0, nodeWidth = 30,
        height = 512, width = 866,
        colourScale = JS(
          'd3.scaleOrdinal()
       .domain(["Esquerda", "Centro", "Direita"])
       .range(["#e2030a", "#0a6b00", "#0073e6"])'
        )
      )
      
      legenda_html <- htmltools::tags$div(
        style = "font-family: sans-serif; padding: 10px;",
        htmltools::tags$h4("Legenda:", 
                           style = "font-family: Garamond; font-size: 18px"),
        htmltools::tags$ul(
          #style = "list-style: none; padding-left: 0;",
          style = "font-family: Garamond; font-size: 14px; list-style: none; padding-left: 0;",
          htmltools::tags$li(htmltools::tags$span(style="color: #e2030a;", "■ "), "Esquerda"),
          htmltools::tags$li(htmltools::tags$span(style="color: #0a6b00;", "■ "), "Centro"),
          htmltools::tags$li(htmltools::tags$span(style="color: #0073e6;", "■ "), "Direita") ) )
      
      # Título superior
      titulo_html <- tags$h2(
        "Migração Ideológica de Deputados Reeleitos (1998–2022)",
        style = "font-family: Garamond; text-align: center;"
      )
      
      # Layout em flex: título acima, sankey + legenda lado a lado
      grafico_completo <- browsable(
        tags$div(
          titulo_html,
          tags$div(
  ...
  # 3. Função principal para gerar Sankey por sigla de partido com cores fixas
  {
    gerar_sankey_Sigla_Partido_colorido <- function(df) {
      
      # Criar pares ideológicos consecutivos por deputado
      df_pairs <- df |>
        group_by(Nome_Deputado) |>
        mutate(origem = lag(Sigla_Partido),
               destino = Sigla_Partido,
               ano_origem = lag(Ano_Eleição),
               ano_destino = Ano_Eleição) |>
        filter(!is.na(origem), ano_destino - ano_origem == 4) |>
        ungroup() |>
      ...
      # Extrair Sigla_Partido de cada node
      nodes$Sigla_Partido <- gsub(" em .*", "", nodes$name)
      
      # Atribuir cor com base na Sigla_Partido
      # Atribuir espectro ideológico a cada partido
      nodes$espectro <-  dplyr::case_when(
        nodes$Sigla_Partido %in% c("PT", "PC DO B", "PDT", "PSOL") ~ "Esquerda",
        nodes$Sigla_Partido %in% c("PPS", "CIDADANIA", "PMDB", "MDB", "PSB", "PV", "REDE", "PSDB") ~ "Centro",
        nodes$Sigla_Partido %in% c("PT DO B", "AVANTE", "PSDC", "DC", "PFL", "DEM", "NOVO", "PAN", "PEN",
                                   "PATRIOTA", "PHS", "PR", "PL", "PMN", "PTN", "PODE", "PPB", "PP", "PPL",
                                   "PRB", "REPUBLICANOS", "PRONA", "PROS", "PRP", "PRTB", "PSC", "PSD",
                                   "PSL", "PST", "PTB", "PTC", "SD", "SOLIDARIEDADE", "UNIÃO") ~ "Direita" )
      
      
      # Atribuir cor com base no espectro
      nodes$color <- dplyr::recode(nodes$espectro,
                                   Esquerda = "#e2030a",
                                   Centro   = "#0a6b00",
                                   Direita  = "#0073e6")
      ...
      df_pairs$group <- nodes$espectro[df_pairs$source_id + 1]
      #df_pairs$group <- nodes$Sigla_Partido[df_pairs$source_id + 1]
      ...
      sankey <- sankeyNetwork(
        ...
        Source = "source_id", Target = "target_id",
        Value = "value", NodeID = "name",
        NodeGroup = "espectro",   # <- aqui você associa o espectro ideológico
        LinkGroup = "group",      # <- isso também deve ser espectro, se quiser cor no link
     ...
     .range(["#e2030a", "#0a6b00", "#0073e6"])'
      ...
      legenda_html <- htmltools::tags$div(
        style = "font-family: Garamond; font-size: 14px; padding: 10px;",
        htmltools::tags$h4("Legenda por Espectro Ideológico:"),
        htmltools::tags$ul(style = "list-style: none; padding-left: 0;",
                           htmltools::tags$li(
                             htmltools::tags$span(style = "color: #e2030a;", "■ "),
                             "Esquerda: PT, PC DO B, PDT, PSOL"
                           ),
                           htmltools::tags$li(
                             htmltools::tags$span(style = "color: #0a6b00;", "■ "),
                             "Centro: PPS, CIDADANIA, PMDB, MDB, PSB, PV, REDE"
                           ),
                           htmltools::tags$li(
                             htmltools::tags$span(style = "color: #0073e6;", "■ "),
                             paste0("Direita: PT DO B, AVANTE, PSDC, DC, PFL, DEM, NOVO, PAN, PEN, PATRIOTA, PHS, PR, PL, ",
                                    "PMN, PTN, PODE, PPB, PP, PPL, PRB, REPUBLICANOS, PRONA, PROS, PRP, PRTB, PSC, PSD, ",
                                    "PSDB, PSL, PST, PTB, PTC, SD, SOLIDARIEDADE, UNIÃO")
                           )
        )
      )
  ...
  # 4. Função principal para gerar Sankey por Tamanho com cores fixas
  {
    gerar_sankey_tamanho_colorido <- function(df) {
      
      # Limpar e padronizar a coluna Tamanho
      df <- df |>
        mutate(Tamanho = str_trim(Tamanho),  # Remove espaços extras
               Tamanho = str_to_title(Tamanho))  # Padroniza capitalização
      ...
      # Extrair tamanho de cada node (já está no formato correto)
      nodes$tamanho <- gsub(" em .*", "", nodes$name)
      
      # Atribuir cor com base no tamanho (corrigido)
      nodes$color <- recode(nodes$tamanho,
                            Grande = "#e2030a",
                            Médio   = "#0a6b00",
                            Pequeno  = "#0073e6",
                            Nanico = "#ff6d0f")
      ...
      # Corrigir referência para group (usar tamanho, não ideologia)
      df_pairs$group <- nodes$tamanho[df_pairs$source_id + 1]
      
      # Criar sankey com estilo (JavaScript corrigido)
      sankey <- sankeyNetwork(
        Links = df_pairs,
        Nodes = nodes,
        Source = "source_id", Target = "target_id",
        Value = "value", NodeID = "name",
        NodeGroup = "tamanho",  # Corrigido
        LinkGroup = "group",
        fontSize = 0, nodeWidth = 30,
        height = 512, width = 866,
        colourScale = JS(
          'd3.scaleOrdinal()
       .domain(["Grande", "Médio", "Pequeno", "Nanico"])
       .range(["#e2030a", "#0a6b00", "#0073e6", "#ff6d0f"])'
        )
      )
      ...
      legenda_html <- htmltools::tags$div(
        ...
        htmltools::tags$ul(
          style = "font-family: Garamond; font-size: 14px; list-style: none; padding-left: 0;",
          htmltools::tags$li(htmltools::tags$span(style="color: #e2030a;", "■ "), "Grande"),
          htmltools::tags$li(htmltools::tags$span(style="color: #0a6b00;", "■ "), "Médio"),
          htmltools::tags$li(htmltools::tags$span(style="color: #0073e6;", "■ "), "Pequeno"),
          htmltools::tags$li(htmltools::tags$span(style="color: #ff6d0f;", "■ "), "Nanico")
        )
      )
      
      # Título superior
      titulo_html <- tags$h2(
        "Migração Por Tamanho de Partido de Deputados Reeleitos (1998–2022)",
        style = "font-family: Garamond; text-align: center;"
      )
    ...
    sankey_plot_tam <- gerar_sankey_tamanho_colorido(sankey)
    
    saveWidget(sankey_plot_ideo, file = "sankey_ideologia.html", selfcontained = TRUE)
    webshot("sankey_ideologia.html", file = "sankey_ideologia.png", vwidth = 3000, vheight = 1800, zoom = 2)
...
#Criar tabelas
{
  # 1. Filtrar só os deputados reeleitos
  df_migrantes <- banco_de_migrações |>
    filter(Reeleitos == "Sim") |>
    arrange(Nome_Deputado, Ano_Eleição) |>
    group_by(Nome_Deputado) |>
    mutate(
      ideologia_origem = lag(Ideologia),
      ideologia_destino = Ideologia,
      ano_origem = lag(Ano_Eleição),
      ano_destino = Ano_Eleição
    ) |>
    filter(!is.na(ideologia_origem), ano_destino - ano_origem == 4) |>
    ungroup()
  
  # 2. Criar tabela cruzada com contagem
  tabela_cruzada <- df_migrantes |>
    count(ideologia_origem, ideologia_destino, name = "Migrantes")
  
  total_geral <- sum(tabela_cruzada$Migrantes)
  
  # 3. Calcular totais por destino
  tabela_completa <- tabela_cruzada |>
    group_by(ideologia_destino) |>
    mutate(
      Total_Destino = sum(Migrantes),
      Percentual = round(100 * Migrantes / total_geral, 1)
    ) |>
    ungroup()
  
  # 4. Transformar em formato de tabela: origem x destino (colunas = destino)
  tabela_percentual <- tabela_completa |>
    select(ideologia_origem, ideologia_destino, Percentual) |>
    pivot_wider(names_from = ideologia_destino, values_from = Percentual, values_fill = 0)
  
  tabela_percentual$Total <- rowSums(tabela_percentual[ , 2:4])
  soma_total <- sum(tabela_percentual$Total)
  ajuste <- 100 - soma_total
  
  # Corrige a célula de maior valor
  linha_mais_alta <- which.max(tabela_percentual$Total)
  tabela_percentual$Total[linha_mais_alta] <- tabela_percentual$Total[linha_mais_alta] + ajuste
  linha_total <- c("Total (N)", colSums(tabela_percentual[ , 2:5]))
  
  # 5. Criar flextable com header mesclado
  ft <- flextable(tabela_completa) |>
    add_footer_row(values = linha_total, colwidths = rep(1, length(linha_total))) |>
    set_header_labels(
      Esquerda = "Esquerda",
      Centro = "Centro",
      Direita = "Direita",
    ...
    add_header_row(
      values = c("", "Ideologia do partido de destino"),
      colwidths = c(1, ncol(tabela_completa) - 1)
    ) |>
    set_header_labels(ideologia_origem = "Ideologia do partido de origem") |>
    theme_vanilla() |>
    align(align = "center", part = "all") |>
    bold(part = "header") |>
    bold(i = 1 == "Total (N)", part = "footer") |>
    hline_top(border = fp_border(width = 2), part = "body") |>
    hline_bottom(border = fp_border(width = 2), part = "footer") |>
    autofit()
